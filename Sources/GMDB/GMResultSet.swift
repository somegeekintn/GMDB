...
public class GMResultSet: NSObject {
    var parentDB: GMDatabase?
    public internal(set) var statement: GMStatement?
    public internal(set) var shouldAutoClose: Bool = false
    public internal(set) var query: String?

    private var _columnNameToIndexMap: [String: Int32]?

    public var sqlStatement: OpaquePointer? { statement?.statement }
    ...
    public var hasAnotherRow: Bool {
        guard let db = parentDB?.sqliteHandle else { return false }

        return sqlite3_errcode(db) == SQLITE_ROW
    }

    public static func resultSet(with statement: GMStatement, usingParentDatabase aDB: GMDatabase, shouldAutoClose: Bool) -> GMResultSet {
        GMResultSet(parentDB: aDB, statement: statement, shouldAutoClose: shouldAutoClose)
    }

    public init(parentDB: GMDatabase, statement: GMStatement, shouldAutoClose: Bool) {
    ...
    public func resultDictionary() -> [String: Any]? {
    ...
    public func nextWithError() throws -> Bool {
    ...
    public func columnIndexFor(column columnName: String) -> Int32 {
    ...
    public func dataFor(column columnName: String) -> Data? {
        dataFor(columnIndex: columnIndexFor(column: columnName))
    }

    public func dataFor(columnIndex: Int32) -> Data? {
        guard validate(columnIndex: columnIndex) else { return nil }
    ...
    public func dataNoCopyFor(column columnName: String) -> Data? {
        dataNoCopyFor(columnIndex: columnIndexFor(column: columnName))
    }

    public func dataNoCopyFor(columnIndex: Int32) -> Data? {
        guard validate(columnIndex: columnIndex) else { return nil }
        let dataBuffer = sqlite3_column_blob(sqlStatement, columnIndex)
        let dataSize = sqlite3_column_bytes(sqlStatement, columnIndex)
    ...
    public func columnIndexIsNull(columnIndex: Int32) -> Bool {
        sqlite3_column_type(sqlStatement, columnIndex) == SQLITE_NULL
    }

    public func columnIsNull(column columnName: String) -> Bool {
        return columnIndexIsNull(columnIndex: columnIndexFor(column: columnName))
    }

    public func utf8StringFor(columnIndex: Int32) -> UnsafePointer<UInt8>? {
        guard validate(columnIndex: columnIndex) else { return nil }

        return sqlite3_column_text(sqlStatement, columnIndex);
    }

    public func utf8StringFor(column columnName: String) -> UnsafePointer<UInt8>? {
        utf8StringFor(columnIndex: columnIndexFor(column: columnName))
    }

    public func objectFor(columnIndex: Int32) -> Any? {
    ...
    public func objectFor(column columnName: String) -> Any? {
        objectFor(columnIndex: columnIndexFor(column: columnName))
    }

    public func columnNameFor(columnIndex: Int32) -> String {
        return String(cString: sqlite3_column_name(sqlStatement, columnIndex))
    }

    public subscript (columnIndex: Int32) -> Any? {
    ...
    public func bind(with arguments: GMDatabase.Arguments) -> Bool {
    ...
    public func bind(with array: [Any?]) -> Bool {
        bind(with: .array(array))
    }

    public func bind(with dictionary: [String: Any?]) -> Bool {
